...
.PHONY: black build clean default deploy deploy-test purge release sign test test-all test-all-verbose test-code test-coverage-all test-coverage-all-verbose test-coverage-verbose test-package test-verbose venv .venv verify
...
test:
	python -m pytest --cov=oc_mirror --cov-report= --log-cli-level info $(args)

test-all:
	python -m pytest --cov=oc_mirror --cov-report= --log-cli-level info --allow-online-modification $(args)

test-all-verbose:
	python -m pytest --cov=oc_mirror --cov-report= --log-cli-level debug --allow-online-modification $(args)

test-code:
	python -m pylint --disable=R0801,W0511 --max-line-length=120 oc_mirror tests
...
test-verbose:
	python -m pytest -r sx --cov=oc_mirror --cov-report= --log-cli-level debug $(args)
